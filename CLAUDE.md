...
Project Atlas는 **AI Workforce OS**다. 고객은 AI 도구를 쓰지 않고 **AI 직원을 채용**한다.
모든 작업은 [`docs/constitution/`](docs/constitution/)의 헌법을 최상위 기준으로 따른다. 헌법과 충돌하면 헌법이 이긴다.

## 절대 규칙

1. **Documentation First.** 코드보다 문서가 먼저다. 설계 결정은 `docs/adr/`에 ADR로 남긴다.
2. **Business First.** 모든 기능은 [BUSINESS_CONSTITUTION](docs/constitution/BUSINESS_CONSTITUTION.md)의 4개 게이트를 통과해야 한다:
...
- 새 기능 아이디어 → 먼저 `docs/business/feature-gate.md` 체크리스트로 검증.
