...
orgNode:
  id: node_xx
  companyId: com_01
  kind: company | ceo | department | employee   # 확장: team, role ...
  refId: <대상 객체 id>                    # Company/CEO/Department/Employee id
  parentId: <상위 node id | null>
  children: [node ids]
```
- 트리 루트 = `kind: company`.
- **CEO 노드**는 company의 자식(개정 #003). 부서는 company의 자식, 직원은 department의 자식.
